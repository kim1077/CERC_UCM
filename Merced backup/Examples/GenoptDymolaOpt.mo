...
realString(number=int.y, minimumWidth=1, precision=16), resultFileName);
  ...
  connect(weather.y,centralPlant. w) annotation (points=[-31.25,85; -23.3,85; 
        -23.3,54.8; -13.6,54.8],
                    style(color=74, rgbcolor={0,0,127}));
  annotation (Diagram);
  connect(CT_Tref.y[1], centralPlant.u[1]) 
                                      annotation (points=[-71,20; -39.3,20; 
        -39.3,44.0667; -13.6,44.0667], style(color=74, rgbcolor={0,0,127}));
  connect(mdot_CHWS.y[1], centralPlant.u[2]) 
                                      annotation (points=[-71,46; -40,46; -40,
        45.6; -13.6,45.6], style(color=74, rgbcolor={0,0,127}));
  connect(T_CHWS_ref.y[1], centralPlant.u[3]) 
                                      annotation (points=[-71,72; -40,72; -40,
        47.1333; -13.6,47.1333], style(color=74, rgbcolor={0,0,127}));
  connect(centralPlant.P, int.u) annotation (points=[33.44,54.8; 43.72,54.8;
        43.72,54; 56.8,54], style(color=74, rgbcolor={0,0,127}));
end GenoptDymolaOpt;
