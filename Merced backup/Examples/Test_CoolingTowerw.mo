model Test_CoolingTowerw 
  ...
  Modelica_Fluid.Sources.PrescribedBoundary_pTX CWR(
    ...
    p=0,
    T=273 + 5) 
    annotation (extent=[34,24; 26,32], rotation=0);
  Modelica_Fluid.Sources.PrescribedMassFlowRate_TX CWS(
    redeclare package Medium = MediumCW,
    m_flow=430,
    T=273.15 + (95 - 32)*5/9) 
    annotation (extent=[-62,22; -50,34]);
  Subsystems.Cooling_Towerw cooling_Tower(
                                         redeclare package Medium = MediumCW,
      Nw=4)       annotation (extent=[-20,8; 16,48]);
  ...
  Modelica.Blocks.Sources.CombiTimeTable weather(
    ...
    startTime=-90*24*3600) annotation (extent=[-90,34; -70,54]);
equation 
  connect(CWS.port, cooling_Tower.CWR) annotation (points=[-50,28; -22.16,28],
      style(color=69, rgbcolor={0,127,255}));
  connect(cooling_Tower.CWS, CWR.port) annotation (points=[18.16,28; 26,28],
      style(color=69, rgbcolor={0,127,255}));
  connect(cooling_Tower.w, weather.y) annotation (points=[-23.6,44; -69,44],
      style(color=74, rgbcolor={0,0,127}));
  connect(pulse.y, cooling_Tower.Fair) annotation (points=[-77.2,8; -51.6,8;
        -51.6,36; -23.6,36], style(color=74, rgbcolor={0,0,127}));
