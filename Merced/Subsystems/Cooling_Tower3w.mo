...
Components.YorkCalc_PrescribedTemp tow(
  ...
  Modelica.Blocks.Routing.DeMultiplex3 deMultiplex3_1
    annotation (Placement(transformation(extent={{6,73},{18,87}}, rotation=0)));
  Buildings.Utilities.Psychrometrics.WetBulbTemperature T_wb(redeclare package
    ...
    annotation (Placement(transformation(extent={{30,67},{50,87}}, rotation=0)));
...
equation
  ...
  connect(T_wb.TDryBul, deMultiplex3_1.y1[1])       annotation (Line(points={{
          31,85},{25,85},{25,84.9},{18.6,84.9}}, color={0,0,255}));
  connect(T_wb.p, deMultiplex3_1.y3[1])       annotation (Line(points={{31,77},
          {26,77},{26,75.1},{18.6,75.1}}, color={0,0,255}));
  connect(deMultiplex3_1.y2[1], T_wb.phi)       annotation (Line(points={{18.6,
          80},{34,80},{34,84},{49,84}}, color={0,0,127}));
  connect(T_wb.TWetBul, tow.TAir)        annotation (Line(points={{49,77},{58,
          77},{58,58},{-22,58},{-22,4},{-14,4}}, color={0,0,255}));
  connect(extractSignal.y, deMultiplex3_1.u)
    annotation (Line(points={{-21,80},{4.8,80}}, color={0,0,127}));
  connect(extractSignal.u, w)
    annotation (Line(points={{-44,80},{-118,80}}, color={0,0,127}));
  connect(add.u1, T_wb.TWetBul) annotation (Line(points={{-26,52},{58,52},{58,
          77},{49,77}}, color={0,0,127}));
