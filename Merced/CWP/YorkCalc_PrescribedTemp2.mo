...
model YorkCalc_PrescribedTemp2
  ...
  Modelica.SIunits.Temperature TApp(min=0, nominal=7.5, start=5)
  ...
  Modelica.SIunits.Temperature TRan(min=0, nominal=5, start=5)
    "Range temperature";
  Modelica.SIunits.MassFraction FRWat
    "Ratio actual over design water mass flow ratio";
  Modelica.SIunits.MassFraction FRAir(min=0,max=2,nominal=0.75,start=1)
...
public
  ...
  Modelica.Blocks.Interfaces.RealInput T_CWS_in(
                                     min=275,max=305,nominal=285)
    "T_CWS Reference"                    annotation (Placement(transformation(
